...
tools: ['read', 'agent', 'sequential-thinking', 'github', 'memory']
---
...
When escalating to the user, open a GitHub issue with:
...
Note: the GitHub MCP handles issues and PRs natively. Adding items to a Projects V2 board requires GraphQL mutations (`addProjectV2ItemById`, `updateProjectV2ItemFieldValue`). Use the issue creation tool first, then attempt the project mutation -- if unsupported, log the issue URL in `tasks.md` as a comment for manual board triage.
...
**Rearchitect circuit breaker:** before delegating to the architect on a `needs-rearchitect` signal, count the `handoff-architect.v*.json` files in the phase folder. If the count is 2 or more (meaning two rearchitects have already been attempted), do NOT delegate to the architect. Instead, escalate to HITL: create a GitHub issue (`agent-escalation`, P0, title: `[Phase N.N] Rearchitect loop limit reached`) with a body listing the blocked handoff path, the number of rearchitect attempts, and the last `blockedDetail`. Stop execution. This caps the most expensive loop in the pipeline.

1. **Research** -- Delegate to the researcher agent. Wait for `docs/log/phase-N/handoff-researcher.json` with `"status": "completed"`. The researcher writes KB files and identifies open questions for the architect.
2. **Architect** -- Delegate to the architect agent. Wait for `docs/log/phase-N/handoff-architect.json` with `"status": "completed"`. If the architect escalates ambiguity, stop and ask the user.
3. **Plan** -- Delegate to the planner with the architect handoff path. Planner reads `handoff-architect.json` and writes to `tasks.md`. After the handoff is written, the planner creates GitHub issues immediately -- do not delegate to designer or builder until issue URLs are recorded in `handoff-planner.json`.
4. **Parse phases** -- Read `tasks.md`. Group steps by file overlap. Steps with no overlapping files can run in parallel.
5. **Execute** -- Delegate to builder and/or designer per step. Mark tasks `[-]` in `tasks.md` when delegated. Delegate non-overlapping steps simultaneously. When a builder writes `handoff-builder.json`, read the `status` field and route:

   | `status` | `blockedReason` | Action |
   |---|---|---|
   | `completed` | `null` | Mark task `[x]`, proceed to step 6 for this task's scope |
   | `blocked` | `needs-rearchitect` | Mark task `[~]` in `tasks.md`, delegate to architect with the `blockedDetail` and current handoff path |
   | `blocked` | `missing-dependency` | Mark task `[!]`, resolve dependency, re-delegate to builder |
   ...
   | `blocked` | `needs-human-approval` | Mark task `[!]`. Builder must have written `docs/log/phase-N.N/exception-report.md`. Create a GitHub issue (`agent-escalation`, P0, title: `[Phase N.N] Human approval required`) with body linking to the exception report and blocked handoff. Stop -- do NOT delegate to architect until human resumes. |
6. **Test** -- Delegate to the tester for each completed build task as soon as its `handoff-builder.json` is written. Pass the specific `handoff-builder.json` path and the `handoff-architect.json` path. Multiple tester instances can run in parallel if their scopes (file sets) do not overlap. Wait for `handoff-tester.json` per task.
7. **Quality** -- Delegate to the quality agent. It scores 4 dimensions and fixes everything below 9, re-scoring up to 3 passes. Wait for `handoff-quality.json`. If it escalates a dimension it cannot fix, mark `[!]` in `tasks.md` and relay to user.
8. **Validate** -- Delegate to the validator. Wait for `handoff-validator.json`. Route on the `"verdict"` field:
   - `"PASS"` -- proceed to step 9.
   - `"FAIL"` -- read `blockingIssues`. Mark affected tasks `[!]` in `tasks.md`. Delegate to builder with the specific issues. Re-delegate to quality, then re-delegate to validator. Check `retryCount` in the new validator handoff -- if it reaches 3 (4th total attempt), stop and escalate to user.
