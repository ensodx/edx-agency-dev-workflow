...
allowed-tools: Read, Grep, Glob, Write, mcp__github__create_issue, mcp__github__get_issue
---
...
6. **Create GitHub issues** -- Immediately after writing the handoff, create one issue per logical deliverable group using `mcp__github__create_issue`. Title format: `[Phase N.N] <deliverable name>`. Body: phase name, deliverable description, AC references, log path. Record each URL in the `githubIssues` array of the handoff.
